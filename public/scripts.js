// Random background image loader (desktop only)
      ...
      function useZipCodeSimple(zip) {
        ...
        fetch(`https://api.zippopotam.us/us/${zip}`)
          ...
          .then((data) => {
            ...
            console.log('🕒 Extracted ZIP location data for recent searches:', { city, state });
      ...
      function searchAndUseLocation(query) {
        ...
        fetch(url, {
          headers: {
            "User-Agent":
              "Nature Events App (github.com/richmalloy/nature-events)",
          },
        })
          .then((res) => {
          ...
          .then((data) => {
            ...
            if (data && data.length > 0) {
              ...
              console.log('🕒 Extracted location data for recent searches:', { city, state, country });
              
              if (city && state) {
                window.recentSearches.addSearch(city, state, country);
              } else {
                console.log('🕒 Skipping recent search - missing city or state');
                console.log('🕒 Available address fields:', Object.keys(location.address));
              }
            ...
            fetch(
              ...
              .then((data) => {
                ...
                if (!cityName || cityName === "location") {
                  cityName =
                    address.municipality || address.county || address.region;
                }
                ...
                if (cityName && stateName) {
                  ...
                  console.log('🕒 Saving geolocation to recent searches:', { cityName, stateName, country });
      ...
      function loadArchaeologyFallback(lat, lng, archEl) {
        ...
        const sites = [
          // Southwest USA
          {
            ...
            lng: -107.9609,
            description:
              "Major ancestral Puebloan cultural center (900-1150 CE)",
          },
          {
            name: "Mesa Verde",
            lat: 37.1853,
          ...
          // Midwest USA
          {
            ...
            lng: -90.0634,
            description:
              "Mississippian culture ceremonial center (1050-1200 CE)",
          },
          {
            name: "Spiro Mounds",
            lat: 35.2431,
      ...
      class RecentSearches {
        ...
        debugCommunityData() {
          try {
            const allCommunityData = JSON.parse(localStorage.getItem('community-searches') || '[]');
            console.log('🔍 DEBUG: ALL COMMUNITY DATA (including your own):');
            console.log('🔍 Total searches in community-searches:', allCommunityData.length);
            allCommunityData.forEach((search, index) => {
              console.log(`🔍 ${index + 1}. ${search.displayName} (sessionId: ${search.sessionId}, timestamp: ${search.timestamp})`);
            });
            
            const currentSessionId = this.getSessionId();
            const yourSearches = allCommunityData.filter(s => s.sessionId === currentSessionId);
            const otherSearches = allCommunityData.filter(s => s.sessionId !== currentSessionId);
            
            console.log(`🔍 Your searches (${currentSessionId}):`, yourSearches.length);
        ...
        showAllCommunityData() {
          try {
            ...
            let html = '<div style="background: #333; padding: 10px; border-radius: 5px; margin-bottom: 10px;"><strong>🔍 DEBUG: All Community Data (including your own)</strong></div>';
            
            allCommunityData.forEach((search, index) => {
              const isYours = search.sessionId === currentSessionId;
              const ago = this.getTimeAgo(search.timestamp);
              html += `
                <div class="search-item" style="background: ${isYours ? '#004400' : '#000044'}; margin: 5px 0; padding: 8px; border-radius: 4px;">
        ...
        addSearch(city, state, country) {
          ...
          searches = searches.filter(search => 
            !(search.city === city && search.state === state && search.country === country)
          );
        ...
        async saveToCommunity(searchData) {
          try {
            ...
            const existingCommunity = JSON.parse(localStorage.getItem('community-searches') || '[]');
            console.log('🌍 Existing community before save:', existingCommunity.length, 'searches');
            
            const updatedCommunity = [communityData, ...existingCommunity].slice(0, 20);
            localStorage.setItem('community-searches', JSON.stringify(updatedCommunity));
            
            console.log('🌍 Successfully saved to community storage');
            console.log('🌍 Updated community now has:', updatedCommunity.length, 'searches');
        ...
        displayPersonalSearches() {
          ...
          if (!container || !section) {
            console.error('🕒 Missing DOM elements - container:', !!container, 'section:', !!section);
            return;
          }
          ...
          console.log('🕒 Found searches - showing section with style.display = block');
          ...
          html += '<button class="clear-searches" onclick="window.recentSearches.clearPersonalSearches()">Clear Recent</button>';
        ...
        testWidget() {
          ...
          const section = document.getElementById('recent-searches');
          console.log('🕒 Section display:', section ? window.getComputedStyle(section).display : 'not found');
          console.log('🕒 Section classList:', section ? Array.from(section.classList) : 'not found');
        ...
        loadPersonalSearches() {
          ...
          const section = document.getElementById('recent-searches');
          if (section) {
            console.log('🕒 Final state - has-data class:', section.classList.contains('has-data'));
            console.log('🕒 Final state - computed display:', window.getComputedStyle(section).display);
          }
        }
        ...
        displayCommunitySearches() {
          const container = document.getElementById('community-searches');
          const communitySection = document.getElementById('community-recent-searches');
        ...
        getActivityFromObservation(obs) {
          ...
          } else if (taxonName.includes('butterfly') || taxonName.includes('moth') || commonName.includes('butterfly')) {
            return 'butterfly spotting';
          } else if (taxonName.includes('flower') || taxonName.includes('plant') || commonName.includes('flower')) {
            return 'wildflower hunting';
          } else if (taxonName.includes('tree') || commonName.includes('tree')) {
            return 'tree identification';
          } else if (taxonName.includes('mammal') || commonName.includes('deer') || commonName.includes('bear')) {
            return 'wildlife tracking';
          } else if (taxonName.includes('insect') || commonName.includes('bee') || commonName.includes('beetle')) {
          ...
          } else if (taxonName.includes('fungi') || taxonName.includes('mushroom') || commonName.includes('mushroom')) {
        ...
        generateCommunitySearches() {
          const interestingLocations = [
            { city: "Yellowstone", state: "Wyoming", country: "United States", activity: "geyser watching", ago: "2h" },
            { city: "Yosemite Valley", state: "California", country: "United States", activity: "waterfall hiking", ago: "4h" },
            { city: "Grand Canyon", state: "Arizona", country: "United States", activity: "sunrise viewing", ago: "6h" },
            { city: "Zion", state: "Utah", country: "United States", activity: "slot canyon exploring", ago: "8h" },
            { city: "Glacier", state: "Montana", country: "United States", activity: "wildlife spotting", ago: "12h" },
            { city: "Acadia", state: "Maine", country: "United States", activity: "coastal tide pooling", ago: "1d" },
            { city: "Olympic", state: "Washington", country: "United States", activity: "rainforest walking", ago: "1d" },
            { city: "Great Smoky Mountains", state: "Tennessee", country: "United States", activity: "wildflower viewing", ago: "2d" },
            { city: "Death Valley", state: "California", country: "United States", activity: "stargazing", ago: "2d" },
            { city: "Everglades", state: "Florida", country: "United States", activity: "bird watching", ago: "3d" },
            { city: "Crater Lake", state: "Oregon", country: "United States", activity: "volcanic lake viewing", ago: "3d" },
            { city: "Bryce Canyon", state: "Utah", country: "United States", activity: "hoodoo formations", ago: "4d" },
            { city: "Sequoia", state: "California", country: "United States", activity: "giant tree viewing", ago: "5d" },
            { city: "Rocky Mountain", state: "Colorado", country: "United States", activity: "alpine hiking", ago: "5d" },
            { city: "Badlands", state: "South Dakota", country: "United States", activity: "fossil hunting", ago: "1w" },
            { city: "Joshua Tree", state: "California", country: "United States", activity: "desert exploring", ago: "1w" },
            { city: "Denali", state: "Alaska", country: "United States", activity: "mountain viewing", ago: "2w" },
            { city: "Haleakala", state: "Hawaii", country: "United States", activity: "volcano sunrise", ago: "2w" }
          ];
        ...
        renderCommunitySearches(searches, container) {
          ...
          searches.forEach(search => {
            const displayName = search.country === 'United States' 
            ...
            html += `
              <div class="search-item community-item" onclick="window.recentSearches.selectSearch('${search.city}', '${search.state}', '${search.country}')">
                ...
                <span class="search-time">${search.ago} ago</span>
              </div>
            `;
          });
          
          html += '<p class="community-note"><small>🌲 Join fellow explorers discovering nature across America!</small></p>';
